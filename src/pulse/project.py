"""Pulse CLI - Project init module.

v1.1.0
"""

from datetime import datetime
from pathlib import Path

from .config import (
    find_project_root, get_pulse_dir, get_async_dir,
    get_queue_sla, load_config, save_config, GLOBAL,
    DEFAULT_QUEUES, DEFAULT_AGENTS, DEFAULT_DAILY_ORDER, DEFAULT_EMOJI,
)
from .sync import sync_to_async


def ensure_pulse_structure(project_root: Path):
    """Create all required .pulse/ and async/ subdirectories."""
    pulse = get_pulse_dir(project_root)
    (pulse / "queues").mkdir(parents=True, exist_ok=True)
    (pulse / "processing").mkdir(parents=True, exist_ok=True)
    (pulse / "locks").mkdir(parents=True, exist_ok=True)
    (pulse / "decisions").mkdir(parents=True, exist_ok=True)
    (pulse / "history").mkdir(parents=True, exist_ok=True)
    (pulse / "events").mkdir(parents=True, exist_ok=True)

    async_dir = get_async_dir(project_root)
    async_dir.mkdir(parents=True, exist_ok=True)

    queue_sla = get_queue_sla(project_root)
    for q in queue_sla:
        (pulse / "queues" / q).mkdir(parents=True, exist_ok=True)
        (async_dir / q).mkdir(parents=True, exist_ok=True)


def cmd_project_init(project_name: str):
    """Initialize a new project with Pulse directory structure."""
    project_root = find_project_root(project_name)
    if project_root is None:
        projects_dir = Path.home() / ".openclaw" / "workspace" / "projects" / project_name
        if not GLOBAL.dry_run:
            projects_dir.mkdir(parents=True, exist_ok=True)
            (projects_dir / "AGENTS.md").write_text(
                f"# {project_name}\n\n协作项目于 {datetime.now().isoformat()} 初始化。\n"
            )
        project_root = projects_dir

    ensure_pulse_structure(project_root)

    base_config = load_config(project_root)

    if base_config.get("initialized_at"):
        from .pulse import out
        out(f"Project '{project_name}' already initialized.", command="project init", idempotent=True)
        return

    if not GLOBAL.dry_run:
        new_config = dict(base_config)
        new_config["name"] = project_name
        new_config["initialized_at"] = datetime.now().isoformat()
        new_config["queues"] = DEFAULT_QUEUES
        new_config["agents"] = DEFAULT_AGENTS
        new_config["daily_summary_order"] = DEFAULT_DAILY_ORDER
        new_config["issue_status_emoji"] = DEFAULT_EMOJI
        save_config(project_root, new_config)
        cfg_file = get_pulse_dir(project_root) / "config.json"
        sync_to_async(cfg_file, project_root)

    from .pulse import out
    out(f"Project '{project_name}' initialized at {project_root}.", command="project init")
