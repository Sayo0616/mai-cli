"""Pulse CLI - Issue commands module.
...
from .config import (
    ...
    get_pulse_dir, get_async_dir, GLOBAL,
)
...
def next_issue_id(project_root: Path, queue: str) -> str:
    ...
    queue_dir = get_pulse_dir(project_root) / "queues" / queue
...
def read_issue(project_root: Path, issue_id: str) -> Optional[Dict[str, Any]]:
    ...
    if not (pulse / "queues").exists():
        return None
    for queue_dir in (pulse / "queues").iterdir():
...
def issue_file_path(project_root: Path, queue: str, issue_id: str) -> Path:
    return get_pulse_dir(project_root) / "queues" / queue / f"{issue_id}.md"
...
def cmd_issue_new(project_root: Path, queue: str, title: str, ref: Optional[str]):
    from .pulse import out, err, ensure_pulse_structure
...
def cmd_issue_claim(project_root: Path, issue_id: str):
    from .pulse import out, err
    agent = os.environ.get("PULSE_AGENT", os.environ.get("AGENT_NAME", "unknown"))
