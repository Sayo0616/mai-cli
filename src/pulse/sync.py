"""Pulse CLI - Async mirror sync module.
...
from .config import get_pulse_dir, get_async_dir, get_queue_sla, GLOBAL


def sync_to_async(src_path: Path, project_root: Path, target_queue: str = ""):
    """Sync a .pulse file to the async mirror.
    ...
    try:
        rel = src_path.relative_to(pulse)
    except ValueError:
    ...
    if top == "processing":
        ...
        if target_queue:
            ...
            for q in queue_sla:
                q_path = pulse / "queues" / q / f"{issue_id}.md"
                if q_path.exists():
                    found_queue = q
                    break
            if found_queue:
