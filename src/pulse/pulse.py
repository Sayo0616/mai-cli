"""Pulse CLI - Main entry point with argument parsing and dispatch.
...
from .config import (
    get_pulse_dir, get_async_dir, find_project_root,
...
def ensure_pulse_structure(project_root: Path):
    """Create all required .pulse/ and async/ subdirectories."""
    from .config import get_queue_sla
    pulse = get_pulse_dir(project_root)
    (pulse / "queues").mkdir(parents=True, exist_ok=True)
    (pulse / "processing").mkdir(parents=True, exist_ok=True)
    (pulse / "locks").mkdir(parents=True, exist_ok=True)
    (pulse / "decisions").mkdir(parents=True, exist_ok=True)
    (pulse / "history").mkdir(parents=True, exist_ok=True)
    (pulse / "events").mkdir(parents=True, exist_ok=True)
    async_dir = get_async_dir(project_root)
    async_dir.mkdir(parents=True, exist_ok=True)
    queue_sla = get_queue_sla(project_root)
    for q in queue_sla:
        (pulse / "queues" / q).mkdir(parents=True, exist_ok=True)
...
def build_parser():
    parser = argparse.ArgumentParser(prog="pulse", description="Agent Collaboration CLI")
...
def dispatch(args):
    ...
    if args.subcommand != "project" or args.proj_cmd != "init":
        ...
        if project_root is None:
            err("Project not found. Set AGENTS_PROJECT/PULSE_PROJECT or run 'pulse project init'.",
                4, error="PROJECT_NOT_FOUND")
        pulse_cfg = get_pulse_dir(project_root) / "config.json"
        if not pulse_cfg.exists():
            err(f"Project not initialized. Run 'pulse project init {project_root.name}'.",
