"""Pulse CLI - Configuration module.
...
def get_config(project_root: Path) -> Dict[str, Any]:
    """Load config from .pulse/config.json, merged with defaults. Keyed by project_root."""
    ...
    cfg_file = pulse_dir / "config.json"
...
def find_project_root(project_name: Optional[str] = None) -> Optional[Path]:
    if project_name:
        for env_var in ["AGENTS_PROJECT", "PULSE_PROJECT"]:
...
def get_pulse_dir(project_root: Path) -> Path:
    return project_root / ".pulse"
