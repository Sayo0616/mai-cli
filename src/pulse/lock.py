"""Pulse CLI - Lock protocol module.
...
from .config import (
    get_pulse_dir, get_async_dir, get_heartbeat_intervals, GLOBAL,
)
from .sync import sync_to_async


def lock_path(project_root: Path, issue_id: str) -> Path:
    return get_pulse_dir(project_root) / "locks" / f"{issue_id}.lock"
...
def acquire_lock(project_root: Path, issue_id: str, agent: str) -> bool:
    ...
    if not GLOBAL.dry_run:
        proc_dir = get_pulse_dir(project_root) / "processing"
...
def release_lock(project_root: Path, issue_id: str):
    ...
    if lp.exists() and not GLOBAL.dry_run:
        lp.unlink()
    proc_file = get_pulse_dir(project_root) / "processing" / f"{issue_id}.md"
...
def cmd_lock_check(project_root: Path, issue_id: str):
    from .pulse import out, out_json, GLOBAL
...
def cmd_lock_force_release(project_root: Path, issue_id: str):
    from .pulse import out, err, write_history, GLOBAL
    lock_info = check_lock(project_root, issue_id)
    if not lock_info:
...
def cmd_lock_guardian(project_root: Path):
    from .pulse import out, out_json, write_history, GLOBAL
    pulse = get_pulse_dir(project_root)
    locks_dir = pulse / "locks"
