"""Pulse CLI - Daily summary module.
...
from .config import (
    get_pulse_dir, get_async_dir, get_daily_order, GLOBAL,
    DAILY_SUMMARY_ORDER,
)
...
def _read_daily_event(project_root: Path) -> Dict[str, Any]:
    ...
    event_file = pulse / "events" / DAILY_EVENT_FILE
...
def daily_summary_write(project_root: Path, agent: str, content: str):
    ...
    pulse = get_pulse_dir(project_root)
    lock_file = pulse / "locks" / DAILY_LOCK_FILE
    ...
    if not GLOBAL.dry_run:
        ...
        with open(lock_file, "w") as f:
            ...
            # Step 5: Write summary
            summary_dir = pulse / "history" / f"daily-{today}"
